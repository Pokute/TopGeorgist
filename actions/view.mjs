...
const create = (viewId, followTgoId = undefined, setAsDefault = false) => {
	...
	if (followTgoId) {
		...
		statsBody.appendChild(createStatsRow('Pos', state => getTgo(state, followTgoId), tgo => `x:${tgo.position.x} y:${tgo.position.y}`));
		statsBody.appendChild(createStatsRow('MovPos', state => getTgo(state, followTgoId), tgo => `x:${tgo.position.x} y:${tgo.position.y}`));
	}
